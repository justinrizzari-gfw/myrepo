...
######HANNAH NOTE: until this point the query is the same as the query to answer the first encounters questions

####Gather authorizataion info relevant to NPFC and when the vessels are transmitting AIS during the time period of interest
init_npfc_auth as (SELECT
                    *
                      FROM(
                        SELECT
                        ...
                        last_timestamp
                        FROM
                        --HANNAH NOTE: CRITICAL to use most recent version of tables and to be CONSISTENT among tables used
                        `vessel_database.all_vessels_v20201201`,
                        UNNEST(registry),
                        UNNEST(activity),
                        UNNEST(feature.geartype)
                        WHERE
                        --There are different registry records within each of these RFMOS 
                        --HANNAH note: just focus on registries of interest
                        (
                          list_uvi LIKE "%NPFC%"
                         )
                        AND
...
carrier_authorization as(
  ...
  neighbor_shipname as fishing_shipname,
  neighbor_imo as fishing_imo,
  neighbor_callsign as fishing_callsign,
  neighbor_flag as fishing_flag,
  neighbor_label as fishing_label,
  event_start,
  event_end,
  mean_lat,
  mean_lon,
  ...
  median_speed_knots,
  event_duration_hr,
  carrier_authorized_from,
  carrier_authorized_to,
  carrier_reg,
  ...
  --HANNAH note: the below line was initially incorrectly assigned to be called WCPFC which is a good example of how typos can have large consequences in terms of ----interpretation of results. In this case this is just an example of what you can do if there are multiple versions of a registry, there is actually only one ------NPFC registry, but others have multiple sources. For example, for WCPFC there is WCPFC (the current registry updated online) and WCPFC2 (excel list of -----------historical registries that we also scrape).
  CASE
  WHEN carrier_reg IN ('NPFC','NPFC2', 'NPFC3') then 'NPFC'
  ...
  --are specifically for the time range of interest when we are looking at the encounters (2018) and are correctly for when the vessel was transmitting AIS during the same time period
  LEFT JOIN(
    SELECT
    auth_ssvid,
...
group by carrier_shipname,carrier_ssvid
