...
-- How many of the carriers from question 2 were authorized by IOTC or CCSBT 
-- during the time of their loitering events?
...
WITH
  ...
  carrier_vessel_info AS (
    SELECT
      ...
      first_timestamp,
      last_timestamp
    FROM `pipe_ais_v3_published.identity_core_v20240401`
    WHERE 
      is_carrier = TRUE
  ...
  ----------------------------------------------------------
  -- Pull authorization information
  ----------------------------------------------------------
  authorizations AS (
    SELECT
      vessel_record_id,
      ...
      authorized_to,
      source_code
    FROM `pipe_ais_v3_published.identity_authorization_v20240401`
    WHERE
      source_code IN ('IOTC', 'CCSBT')
  ),

  ----------------------------------------------------------
  -- Join with loitering
  ----------------------------------------------------------
  loitering_authorized AS (
    SELECT a.*, b.* EXCEPT(vessel_record_id)
    ...
    ON (
      a.vessel_record_id = b.vessel_record_id
      AND a.loitering_start_timestamp >= b.authorized_from AND a.loitering_end_timestamp <= b.authorized_to
    )
  )
